...
from providers.specs.reference_csv_data import reference_json_data
...
class TestSQLitePersonStore(unittest.TestCase):
    ...
    def test_entity_conversion(self):
        ...
        entity_list = entity_converter.convert_to_entity_list(reference_csv_data)
        self.assertEqual(json.dumps(entity_list[0].__dict__), reference_json_data[0])
        self.assertEqual(json.dumps(entity_list[1].__dict__), reference_json_data[1])

    def test_entity_conversion_with_one_invalid_data(self):
        entity_converter = EntityConverter()
        data_list = list((reference_csv_data[0], {}))
    ...
    def test_save_person_list(self):
        data_list_stats = self.data_store.save_person_list(reference_csv_data)
